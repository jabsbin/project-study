...
# 공통 규격(틀) 클래스 : 모든 배송 클래스는 배송비를 가져야한다라는 규칙
class Delivery(ABC):
    @abstractmethod
...
class NormalDelivey(Delivery): # 일반 배송
    def get_fee(self, distance):
        return 3000

class QuickDelivey(Delivery): # 일반 배송
    def get_fee(self, distance):
...
DeliveryUtil.print_fee(c1, 5) #delivery가 c1이 되고 거라는 5가 된다
print()
