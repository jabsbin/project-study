...
# 1)거리는 나중에 KNN, K-Meams, Embrdding 유사도 등과 연결되고,
# 2)기울기는 선형회귀를 거쳐 미분Gradient Descent, 딥러닝의 학습 원리로 연결된다
...
class CalcTest:
    ...
    # 피타고라스 정리에 따라 두 점 사이의 직선 거리가 직각 삼각형의 빗면이 된다.
    def distance(self):
        dx = self.x2 - self.x1
        dy = self.y2 - self.y1
    ...
    def transform(self, x_list:list[float]): # 형 변환
...
def main():
    ...
    print('역변환 자료 : ', reversed_data)

if __name__ == "__main__":
    main()

# 클래스 포함 관계는 자신의 것 처럼 사용
# 클래스 상속 관계는 A를 자식으로 두고 B를 부모로 사용 (다형성: 상황에 따라 여러 형태로 변경)